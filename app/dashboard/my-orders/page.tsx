...
import { getUserSession, getMockOrders } from "@/app/auth/actions"
...
export default async function MyOrdersPage() {
  ...
  const orders = await getMockOrders(user.email)
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="container mx-auto px-4 py-12">
        ...
        {orders.length === 0 ? (
          <p className="text-lg text-gray-700">Você ainda não fez nenhum pedido.</p>
        ) : (
          <div className="grid grid-cols-1 gap-6">
            {orders.map((order) => (
              <Card key={order.id}>
                <CardHeader>
                  <CardTitle className="text-xl">Pedido #{order.id.split("_")[1]}</CardTitle>
                </CardHeader>
                <CardContent className="space-y-2">
                  <p>
                    <span className="font-semibold">Produto:</span> {order.productName}
                  </p>
                  <p>
                    <span className="font-semibold">Valor:</span> R$ {typeof order.price === 'number' ? order.price.toFixed(2).replace(".", ",") : 'Indisponível'}
                  </p>
                  <p>
                    <span className="font-semibold">Data:</span> {order.date}
                  </p>
                  <p>
